	...
	<view class="body">
		...
		<view v-else>
			...
			<view class="add_reward" style="margin-top: 40upx;">
				<image src="../../../../../static/img/addStore.png" v-on:click="add_img"></image>
				<text>继续添加广告图片</text>
			</view>
		</view>
				...
				<view v-if="item.modifyDiscount==true">
					<view class="add_reward2">
						...
						<view class="inputs">
							...
							<input type="text" :value="item.probability" placeholder="请输入该券中奖概率" style="width: 70%;border-bottom: 1px solid #F2F2F2;" :id="index + 1"
							 v-on:input="setProbability" />
						</view>
						<view class="inputs">
							<view style="width: 30%;">数量上限：</view>
							<input type="text" :value="item.toplimit" placeholder="请输入该券数量上限" style="width: 70%;border-bottom: 1px solid #F2F2F2;" :id="index + 1"
							 v-on:input="setToplimit" />
						</view>
						<view class="inputs">
							<view style="width: 30%;">发券时间：</view>
							<input type="text" :value="item.publishTime" placeholder="请输入优惠券发送时间" style="width: 70%;border-bottom: 1px solid #F2F2F2;" :id="index + 1"
							 v-on:input="setPTime" />
						</view>
						<view class="inputs">
